...
/**
 * Server-only. Encrypts per-user connector connection keys before they are
 * stored, so the raw credential never sits in a readable database column.
 * Lovable provisions APP_USER_CONNECTION_KEY_SECRET (base64, 32 bytes).
 */
function key(): Buffer {
